...
export default function DeliverScreen() {
  ...
  const uploadProofImage = async (token: string): Promise<string> => {
    ...
    const uploadRes = await fetch(`${API}/rider/upload-proof.php`, {
      ...
      body: JSON.stringify({ image: base64, type: 'delivery' }),
    });
  ...
  const confirmDelivery = async () => {
    if (!photo) { Alert.alert('Required', 'Please take a photo as proof of delivery.'); return; }
    setLoading(true);
    try {
      ...
      const res = await fetch(`${API}/rider/orders.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ order_id: Number(id), action: 'deliver', proof_image: proofUrl }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
      Alert.alert('✓ Delivered', 'Order completed successfully!', [{ text: 'OK', onPress: () => router.replace('/(tabs)' as any) }]);
    } catch (e: any) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      {/* Header */}
      <View style={{ backgroundColor: COLORS.success, paddingTop: insets.top + 8, paddingBottom: 18, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 14 }}>
        <TouchableOpacity onPress={() => router.back()} style={{ width: 40, height: 40, borderRadius: 13, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ color: '#fff', fontSize: 20, fontWeight: '300' }}>‹</Text>
        </TouchableOpacity>
        <View>
          <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700' }}>Delivery Proof</Text>
          <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, marginTop: 2 }}>Order #{id}</Text>
        </View>
      </View>

      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 100, gap: 14 }} showsVerticalScrollIndicator={false}>
        {/* Info */}
        <View style={{ backgroundColor: COLORS.successLight, borderRadius: 16, padding: 18, borderWidth: 1, borderColor: '#a7f3d0' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <Text style={{ fontSize: 20 }}>✅</Text>
            <Text style={{ fontSize: 14, fontWeight: '700', color: '#047857' }}>Confirm Delivery</Text>
          </View>
          <Text style={{ fontSize: 13, color: '#059669', lineHeight: 20, paddingLeft: 30 }}>
            Take a photo showing the order was delivered to the customer.
          </Text>
        </View>
          ...
          {photo ? (
            ...
            <View style={{ gap: 10 }}>
              <TouchableOpacity
                onPress={takePhoto}
                activeOpacity={0.8}
                style={{ flex: 1, minHeight: 200, borderWidth: 2, borderStyle: 'dashed', borderColor: '#a7f3d0', borderRadius: 18, alignItems: 'center', justifyContent: 'center', backgroundColor: '#f0fdf4' }}
              >
                <View style={{ width: 72, height: 72, borderRadius: 36, backgroundColor: COLORS.successLight, alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
      ...
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: COLORS.card, borderTopWidth: 1, borderTopColor: COLORS.border, padding: 16, paddingBottom: insets.bottom + 16 }}>
        <TouchableOpacity
          ...
          activeOpacity={0.85}
          style={{
            backgroundColor: photo ? COLORS.success : COLORS.textLight,
            paddingVertical: 18,
            borderRadius: 16,
            alignItems: 'center',
            ...(photo ? SHADOWS.md : {}),
        ...
        >
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={{ color: '#fff', fontWeight: '700', fontSize: 16 }}>Confirm Delivery</Text>}
        </TouchableOpacity>
