      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-shrink-0">
          <div
            ...
            onClick={() => document.getElementById(`edit-color-input-${idx}`)?.click()}
            title="Pick color"
          />
          <input
...
export default function EditProduct() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [catRes, brgRes, prodRes] = await Promise.all([
          categoriesAPI.list(),
          barangaysAPI.list(),
        ...
        setForm({
          ...
          brand: product.brand || '',
          sku: product.sku || '',
          barangay_id: product.barangay_id ? String(product.barangay_id) : '',
          is_available: product.is_available == 1
        });

        if (product.images?.length > 0) {
          setImages(product.images.map(img => ({ url: img.image_url, preview: img.image_url })));
        }

        if (product.variations?.length > 0) {
          const baseSellerPrice = parseFloat(product.price);
          const colorVars = [];
          const plainVars = [];

          product.variations.forEach(v => {
            const absolutePrice = String((baseSellerPrice + parseFloat(v.price_adjustment)).toFixed(2));
            if (v.name === 'Color') {
              // Color variants don't carry their own price — only name, hex, image
              colorVars.push({
                hex: v.hex || '#3B82F6',
                value: v.value || '',
  ...
  const addColorVariant = () => {
    setColorVariants(prev => [...prev, { hex: '#3B82F6', value: '', image_url: null, imagePreview: null }]);
  };
  ...
  // Stock for each color is synced from plain variant total, or base stock.
  ...
  const handleSubmit = async (e) => {
    ...
    for (const v of colorVariants) {
      if (!v.value.trim()) { showToast('Each color variant must have a color name.', 'warning'); return; }
    }
    for (const v of variations) {
      if (!v.name.trim() || !v.value.trim()) { showToast('Each variant must have a type and value.', 'warning'); return; }
      if (!v.price || parseFloat(v.price) <= 0) { showToast('Each variant must have a valid price.', 'warning'); return; }
    }
          ...
          {variations.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <div className="px-3 py-2.5 border rounded-lg bg-primary-50 border-primary-200 text-sm font-semibold text-primary-800">{totalVariantStock}</div>
        ...
        <div className="bg-white rounded-xl border p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {hasVariants ? 'Price & stock are computed automatically from variants' : 'Optional — add sizes, units, or options'}
              </p>
            </div>
          ...
          {variations.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4 border-2 border-dashed rounded-xl">
              {supportsColorVariants ? 'No other variants. Add sizes or units here.' : 'No variants yet.'}
            </p>
          ) : (
            <div className="space-y-3">
        ...
        <div className="bg-white rounded-xl border p-5 space-y-4">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="is_available" checked={form.is_available}
              ...
              className="w-4 h-4 text-primary-800 rounded border-gray-300 focus:ring-primary-800" />
            <label htmlFor="is_available" className="text-sm text-gray-700">Product is available for purchase</label>
          </div>
        </div>
